...
public class AsignacionesPanel extends JPanel {
    ...
    private void initComponents() {
        ...
        listProyectos.setCellRenderer(new DefaultListCellRenderer() {
        	...
        	public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
        	    ...
        	    if (isSelected) {
        	        ...
        	        if (index % 2 == 0) setBackground(TABLE_ROW);
        	        else setBackground(TABLE_ROW_ALT);
        	    }
        ...
        tablaAsignados = new JTable(modeloTabla) {
        	...
        	public Component prepareRenderer(TableCellRenderer renderer, int row, int col) {
        	    ...
        	    if (isRowSelected(row)) {
        	        ...
        	        else c.setBackground(TABLE_ROW_ALT);
        ...
        tablaAsignados.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
            @Override
            public void valueChanged(ListSelectionEvent e) {
                int row = tablaAsignados.getSelectedRow();
                if (row >= 0) idEmpleadoSeleccionado = Integer.parseInt(modeloTabla.getValueAt(row, 0).toString());
                else idEmpleadoSeleccionado = -1;
            }
        });
        ...
        scrollProyectos.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
    ...
    private JButton gradientBtn(String txt, Color c1Start, Color c1End, Color c2Start, Color c2End) {
    ...
    private void estilizarScrollbar(JScrollBar bar) {
        ...
        bar.setUI(new BasicScrollBarUI() {
            ...
            protected void paintThumb(Graphics g, JComponent c, Rectangle r) {
                ...
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                int barW = 3;
                int x = r.x + (r.width - barW) / 2;
                Color thumbColor;
                if (isThumbRollover()) thumbColor = THUMB_HOV;
                else thumbColor = THUMB_COL;
    ...
    private void asignar() {
        ...
        if (idProyectoSeleccionado == -1) {
            errorMsg = "Selecciona un proyecto primero";
        } else if (cbEmpleados.getSelectedIndex() < 0) {
            errorMsg = "Selecciona un empleado";
        } else if (txtRol.getText().trim().isEmpty()) {
            errorMsg = "El rol es obligatorio";
        } else {
            try {
                Integer.parseInt(txtHoras.getText().trim());
            } catch (NumberFormatException ex) {
                errorMsg = "Las horas deben ser un número entero";
            }
        }
        if (errorMsg != null) {
        ...
        int idEmp = listaEmpleados.get(cbEmpleados.getSelectedIndex()).getIdEmpleado();
        int horas = Integer.parseInt(txtHoras.getText().trim());
        epDAO.asignar(new EmpleadoProyecto(idEmp, idProyectoSeleccionado, txtRol.getText().trim(), horas));
    ...
    private void desasignar() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "¿Desasignar este empleado del proyecto?",
